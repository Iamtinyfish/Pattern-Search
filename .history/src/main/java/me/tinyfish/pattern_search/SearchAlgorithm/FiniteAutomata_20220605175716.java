...
public class FiniteAutomata implements SearchAlgorithm{
    ...
    public List<Integer> search(char[] text, char[] pattern) {
        ...
        int[][] FA = efficientlyBuildAutomata(pattern, distinctChars);
        if (isLog) printFATable(pattern, FA, distinctChars);
//        this.FA = buildAutomata();
//        printFATable();
        ...
        for (int i = 0; i < text.length; ++i) {
            state = FA[state][distinctChars.indexOf(text[i])];
    ...
    public int[][] buildAutomata(char[] pattern, List<Character> distinctChars) {
        ...
        //FA TABLE
        int[][] FA = new int[numOfStates][numOfChars];
        for (int state = 0; state < numOfStates; ++state) {
            for (int col = 0; col < numOfChars; ++col) {
                FA[state][col] = getNextState(pattern,state, distinctChars.get(col));
            }
        }
    ...
    public int[][] efficientlyBuildAutomata(char[] pattern, List<Character> distinctChars) {
        ...
        //FA TABLE
        int[][] FA = new int[numOfStates][numOfChars];
        FA[0][distinctChars.indexOf(pattern[0])] = 1;   //state 0 -> 1
        int lps = 0;
        for (int state = 1; state < numOfStates; ++state) {
            //copy row lps -> row this state
            System.arraycopy(FA[lps], 0, FA[state], 0, numOfChars);

            if (state < pattern.length) {   // not final row
                FA[state][distinctChars.indexOf(pattern[state])] = state + 1;
                lps = FA[lps][distinctChars.indexOf(pattern[state])];
            }
        }
        return FA;
    }

    private void printFATable(char[] pattern, int[][] FA, List<Character> distinctChars) {

        int numOfStates = pattern.length + 1;
        System.out.println("\nPREPROCESS PHASE: FA table");
        System.out.println(distinctChars.);
        for (int i = 0; i < numOfStates; ++i) {
            System.out.print(i + " ");
            System.out.print(Arrays.toString(FA[i]));
